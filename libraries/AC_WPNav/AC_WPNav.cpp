...
/// set_wp_destination waypoint using location class
///     returns false if conversion from location to vector from ekf origin cannot be calculated
bool AC_WPNav::set_wp_destination_smooth(const Location& destination,float width)
{
    bool terr_alt;
    Vector3f dest_neu;
    ...
    // set target as vector from EKF origin
    return set_wp_destination_smooth(dest_neu, terr_alt, width);
}
...
bool AC_WPNav::set_wp_destination_smooth(const Vector3f& destination, bool terrain_alt, float width)
{
    ...
    // set origin and destination
    return set_wp_origin_and_destination_smooth(origin, destination, terrain_alt);
}
...
///     terrain_alt should be true if origin.z and destination.z are desired altitudes above terrain (false if these are alt-above-ekf-origin)
///     returns false on failure (likely caused by missing terrain data)
bool AC_WPNav::set_wp_origin_and_destination_smooth(const Vector3f& origin, const Vector3f& destination, bool terrain_alt)
{
    float throttle_alt_offset = _pos_control.get_throttle_alt_offset();
    ...
    _flags.fast_waypoint = false;   // default waypoint back to slow
    _flags.slowing_down = false;    // target is not slowing down yet
    _flags.segment_type = SEGMENT_SMOOTH;
    _flags.new_wp_destination = true;   // flag new waypoint so we can freeze the pos controller's feed forward and smooth the transition
    _flags.wp_yaw_set = false;
...
/// advance_wp_target_along_track - move target location along track from origin to destination
/// baiyang added in 20190814
bool AC_WPNav::advance_wp_target_along_track_rf(float dt)
{
    float track_covered;        // distance (in cm) along the track that the vehicle has traveled.  Measured by drawing a perpendicular line from the track to the vehicle.
    Vector3f track_error;       // distance error (in cm) from the track_covered position (i.e. closest point on the line to the vehicle) and the vehicle
    float track_desired_max;    // the farthest distance (in cm) along the track that the leash will allow
	...
	float throttle_alt_offset = _pos_control.get_throttle_alt_offset();
    // calculate 3d vector from segment's origin
    Vector3f curr_delta = (curr_pos - Vector3f(0,0,throttle_alt_offset - _throttle_alt_offset_last)) - _origin;
    ...
    if (is_zero(throttle_alt_offset) && \
        ...
        track_error_max_abs = MAX(_track_leash_length*track_error_z/leash_z, _track_leash_length*_track_error_xy/_pos_control.get_leash_xy());
	}else {
    ...
    // do not let desired point go past the end of the track unless it's a fast waypoint
    if (!_flags.fast_waypoint) {
        if (_flags.segment_type == SEGMENT_SMOOTH) {
            _track_desired = constrain_float(_track_desired, 0, _track_length + 3*WPNAV_WP_FAST_OVERSHOOT_MAX);
		}else {
            _track_desired = constrain_float(_track_desired, 0, _track_length);
		}
    } else {
    ...
    Vector3f pos_delta_unit(_pos_delta_unit.x,_pos_delta_unit.y,_pos_delta_unit.z);
    if (!is_zero(throttle_alt_offset) && !is_zero(_pos_delta_unit.z))
    {
        pos_delta_unit.z = 0;
    ...
    // recalculate the desired position
    Vector3f final_target = _origin + pos_delta_unit * _track_desired;
    // convert final_target.z to altitude above the ekf origin
    final_target.z += throttle_alt_offset;

    if (_rangefinder_available && _rangefinder_use) {
        if (_rangefinder_healthy || !is_zero(throttle_alt_offset)) {
    ...
    if( !_flags.reached_destination ) {
        if( _track_desired >= _track_length ) {
            ...
            if (_flags.fast_waypoint) {
                ...
                // regular waypoints also require the copter to be within the waypoint radius
                Vector3f dist_to_dest = (curr_pos - Vector3f(0,0,throttle_alt_offset)) - _destination;
                if(is_zero(throttle_alt_offset) && dist_to_dest.length() <= _wp_radius_cm ) {
                    _flags.reached_destination = true;
                }else if (safe_sqrt(sq(dist_to_dest.x)+sq(dist_to_dest.y)) <= _wp_radius_cm ) {
                    _flags.reached_destination = true;
                }
            }
        }
...
bool AC_WPNav::update_wpnav_rf()
{
    ...
    // advance the target if necessary
    if (!advance_wp_target_along_track_rf(dt)) {
        // To-Do: handle inability to advance along track (probably because of missing terrain data)
        ret = false;
    }
...
void AC_WPNav::calc_slow_down_distance(float speed_cms, float accel_cmss)
{
	...
	if (_flags.segment_type == SEGMENT_SMOOTH) {
        _slow_down_dist = (speed_cms * speed_cms - sq(_smoothing_speed))/ (2.0f*accel_cmss);
	}
}
...
float AC_WPNav::get_slow_down_speed(float dist_from_dest_cm, float accel_cmss)
{
    ...
    if (_flags.segment_type == SEGMENT_SMOOTH) {
		target_speed = safe_sqrt(dist_from_dest_cm * 2.0f * accel_cmss + sq(_smoothing_speed));
	}
