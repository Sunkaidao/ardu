...
char AP_GPS_NMEA_DA :: _check_crc[8]={0};                               ///< Stored CRC check value
uint8_t AP_GPS_NMEA_DA ::  _check_crc_number = 0;                       ///< check_CRC_number index within the current sentence

/// CRC check related parameters   
/// 
unsigned int AP_GPS_NMEA_DA ::  ulTemp1 = 0;  
unsigned int AP_GPS_NMEA_DA ::  ulTemp2 = 0;  
unsigned int AP_GPS_NMEA_DA ::  ulCRC = 0;

//
uint8_t AP_GPS_NMEA_DA ::  _message_type = 0;

bool AP_GPS_NMEA_DA::read(void)
{
    int16_t numc;
    bool parsed = false;
...
bool AP_GPS_NMEA_DA::_decode(char c)
{
...
// internal utilities
//
int16_t AP_GPS_NMEA_DA::_from_hex(char a)
{
    if (a >= 'A' && a <= 'F')
        return a - 'A' + 10;
    else if (a >= 'a' && a <= 'f')
...
int32_t AP_GPS_NMEA_DA::_parse_decimal_100(const char *p)
{
    char *endptr = nullptr;
 ...
 */
uint32_t AP_GPS_NMEA_DA::_parse_degrees()
{
    char *p, *q;
    uint8_t deg = 0, min = 0;
    float frac_min = 0;
 ...
 */
bool AP_GPS_NMEA_DA::_have_new_message()
{
    if (_last_RMC_ms == 0 ||
        _last_GGA_ms == 0) {
        return false;
...
bool AP_GPS_NMEA_DA::_term_complete()
{
    ...
    if (_is_check_term) {
		...
		if (_message_type == _GPS_MESSAGE_NMEA){  
            ...
            checksum =  268435456*_from_hex(_term[0])+		
                        ...
                        _from_hex(_term[7]);
			//gcs().send_text(MAV_SEVERITY_INFO, "NMEA DA: %X %X",_parity,checksum);
        }
        ...
        if (checksum == _parity) {
            if (_gps_data_good) {
                ...
                case _GPS_SENTENCE_VTG:
                    ...
                    break;
                case _GPS_SENTENCE_HDA:
                    _last_HDA_ms  = now;
                    state.gps_yaw = wrap_360(_new_gps_yaw*0.01f - state.declination);
                    state.have_gps_yaw = true;
					state.head_status   = _status;
					//gcs().send_text(MAV_SEVERITY_INFO, "NMEA DA: GPSYAW %f,%f",state.gps_yaw,_new_gps_yaw);
                    break;
                }
            } else {
                switch (_sentence_type) {
    ...
    if (_term_number == 0) {
        ...
        } else if (strcmp(_term, "HEADINGA") == 0 || \
			strcmp(_term, "HEADING3A") == 0) {
            _sentence_type = _GPS_SENTENCE_HDA;
            // HDA doesn't have a data qualifier
            _gps_data_good = true;
    ...
    if (_sentence_type != _GPS_SENTENCE_OTHER && _term[0]) {
		...
		case _GPS_SENTENCE_HDA + 13: // heading (HEADINGA or HEADING3A)		
            _new_gps_yaw = _parse_decimal_100(_term);	
            break;

        case _GPS_SENTENCE_HDA + 11:
 ...
 */
bool
AP_GPS_NMEA_DA::_detect(struct NMEA_DA_detect_state &state, uint8_t data)
{
	switch (state.step) {
	case 0:
		state.ck = 0;
...
uint32_t
AP_GPS_NMEA_DA ::CRC32Value(int i)
{   
	int j;        
	uint32_t _crc = i;
	for ( j = 8 ; j > 0; j-- )  
...
uint32_t
AP_GPS_NMEA_DA ::CalculateBlockCRC32(   unsigned char i)
{  
